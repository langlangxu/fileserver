...
function tellme(req,res){
    ...
    var	fileUrl = (rootPath + pathname).replace(/\\/g,'\/') ;
	...
	fs.readdir(url,function(err,files){
		if(!err) {
			...
			function finished(list){
				...
				var filepath = url.toLowerCase().replace(/\\/g,'/').replace(rootPath.toLowerCase().replace(/\\/g,'/'),'').replace(/^\/|\/$/g,'').split('/')
				var folderpath = '/'
				var txt = 
