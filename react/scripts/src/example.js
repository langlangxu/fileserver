...
var Mainbody = React.createClass({
  ...
  render:function(){
    ...
    var ImageList = imgArr.map(function(item,index){
      return  <Photounit url={item.url} text={item.title} index={index} center={_that.state.center == index} onSelect={_that.select}/>
    })
    return(
...
var Photounit = React.createClass({
  getInitialState:function(){
    return {
      flip:false,
    }
  ...
  render:function(){
    ...
    // console.log(this)
    return (
        <div className={"phone-unit" + (this.props.center&&" select"||"")} style={styleSheet} onClick={this.select}>
            <div className="back-text">反面文字</div>
            <div className="img-box" style={{"background-image":"url("+ this.props.url +")"}}></div>
            <div className="img-title">{this.props.text}</div>
        </div>
      )
  }
})
